...
def multi_gpu_det(model, data_loader, tmpdir=None):
    model.eval()
    results = []
    dataset = data_loader.dataset
...
def single_gpu_seg(model, data_loader, show=False, cfg=None):
    model.eval()
    results = Counter()
    dataset = data_loader.dataset
    prog_bar = mmcv.ProgressBar(len(dataset))
    for i, data in enumerate(data_loader):
        with torch.no_grad():
            result = model(return_loss=False, rescale=not show, ignore_index=cfg.evaluation['ignore_index'], **data)
    ...
    print('pixAcc: {0:.3f} mAcc: {1:.3f} mIoU: {2:.3f}'.format(pixAcc, mAcc, mIoU))
    return results


def multi_gpu_seg(model, data_loader, tmpdir=None):
...
def main():
    ...
    if not distributed:
        ...
        if cfg.model_type == 'detection':
            ...
            outputs = single_gpu_seg(model, data_loader, args.show, cfg)
    else:
        model = MMDistributedDataParallel(model.cuda())
        if cfg.model_type == 'detection':
            outputs = multi_gpu_det(model, data_loader, args.tmpdir)
        else:
            outputs = multi_gpu_seg(model, data_loader, args.tmpdir)
