...
class SegDistEvalAllHook(Hook):
    ...
    def __init__(self,
        ...
        self.best_mIoU = self.pixAcc = self.mAcc = 0
    ...
    def after_train_epoch(self, runner):
        ...
        for idx in range(runner.rank, len(self.dataset), runner.world_size):
            ...
            with torch.no_grad():
                result = runner.model(
                    return_loss=False, ignore_index=self.ignore_index, **data_gpu)
    ...
    def evaluate(self, runner, results):
        ...
        if mIoU > self.best_mIoU:
            ...
            torch.save(state, osp.join(runner.work_dir, 'model_best.pth'))
