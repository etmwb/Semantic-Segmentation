...
int depthaware_conv_forward_cuda(at::Tensor input, at::Tensor depth,
                             at::Tensor offset, at::Tensor output,
                             at::Tensor columns, at::Tensor ones, int kW,
                             int kH, int dW, int dH, int padW, int padH,
                             int dilationW, int dilationH, int group,
  ...
  shape_check(input, NULL, NULL, weight, kH, kW, dH, dW, padH, padW,
              dilationH, dilationW, group, deformable_group);
  AT_CHECK(depth.size(2) == input.size(2) && depth.size(3) == input.size(3),
           "input size should be consistent with depth size, ",
           "but got iH: %d iw: %d dH: %d dW: %d", input.size(2),
           input.size(3), depth.size(2), depth.size(3));
  at::DeviceGuard guard(input.device());
  ...
  // todo: assert batchsize dividable by im2col_step: Done
  ...
  AT_CHECK((offset.size(0) == batchSize), "invalid batch size of offset");
  AT_CHECK(batchSize % im2col_step == 0 && im2col_step < batchSize,
           "batch(%d) must divide im2col_step(%d)", batchSize, im2col_step);
  ...
  input = input.view({batchSize / im2col_step, im2col_step, nInputPlane,
                      inputHeight, inputWidth});
  depth =
      depth.view({batchSize / im2col_step, im2col_step,
  ...
  for (int elt = 0; elt < batchSize / im2col_step; elt++) {
    deformable_im2col(input[elt], depth[elt], nInputPlane, inputHeight,
                      inputWidth, kH, kW, padH, padW, dH, dW, dilationH,
  ...
  input = input.view({batchSize, nInputPlane, inputHeight, inputWidth});
  depth = depth.view(
      {batchSize, 1, outputHeight, outputWidth});

  if (batch == 0) {
    output = output.view({nOutputPlane, outputHeight, outputWidth});
    input = input.view({nInputPlane, inputHeight, inputWidth});
    depth = depth.view({depth.size(1), depth.size(2), depth.size(3)});
  }

  return 1;
}

int depthaware_conv_backward_cuda(at::Tensor input, at::Tensor depth,
                                    at::Tensor gradOutput, at::Tensor gradInput,
                                    at::Tensor gradOffset, at::Tensor weight,
                                    at::Tensor columns, int kW, int kH, int dW,
                                    int dH, int padW, int padH, int dilationW,
                                    int dilationH, int group,
                                    int im2col_step) {
  shape_check(input, NULL, &gradOutput, weight, kH, kW, dH, dW, padH, padW,
              dilationH, dilationW, group, deformable_group);
  at::DeviceGuard guard(input.device());
  ...
  if (input.ndimension() == 3) {
    ...
    batch = 0;
    input = input.view({1, input.size(0), input.size(1), input.size(2)});
    depth = depth.view({1, depth.size(0), depth.size(1), depth.size(2)});
    gradOutput = gradOutput.view(
        {1, gradOutput.size(0), gradOutput.size(1), gradOutput.size(2)});
  }
  ...
  for (int elt = 0; elt < batchSize / im2col_step; elt++) {
    ...
    depthaware_col2im(columns, depth[elt], nInputPlane, inputHeight,
                      inputWidth, kH, kW, padH, padW, dH, dW, dilationH,
  ...
  if (batch == 0) {
    ...
    input = input.view({nInputPlane, inputHeight, inputWidth});
    gradInput = gradInput.view({nInputPlane, inputHeight, inputWidth});
    depth = depth.view({offset.size(1), offset.size(2), offset.size(3)});
  }
